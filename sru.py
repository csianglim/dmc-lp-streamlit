...
VARIABLES = {
    'MV1': {'Name': 'O2', 'UOM': '%', 'Cost': 10, 'SSVal': 30.0, 
            'Limits': (21, 37), 'EngLimits': (21.0, 60.0), 'TYPMOV': 0.1},
    'MV2': {'Name': 'FG', 'UOM': 'SCFH', 'Cost': 0.1, 'SSVal': 900.0, 
            'Limits': (400, 1000), 'EngLimits': (290, 1500), 'TYPMOV': 10},
    'CV1': {'Name': 'P', 'UOM': 'psi', 'SSVal': 8.6, 
            'Limits': (7.0, 9.0), 'EngLimits': (6.0, 10.0), 'TYPMOV': 0.1},
    'CV2': {'Name': 'dP', 'UOM': 'psi', 'SSVal': 10.5, 
            'Limits': (8.0, 14.0), 'EngLimits': (0.0, 20.0), 'TYPMOV': 0.1},
    'CV3': {'Name': 'Temp', 'UOM': "°F", 'SSVal': 2120, 
            'Limits': (2100, 2150), 'EngLimits': (2050, 2200), 'TYPMOV': 1},
    'FF1': {'Name': 'AC155.CO % (H2S-2*SO2)', 'UOM': '%', 'SSVal': 50},
    'FF2': {'Name': 'Amine Acid Gas', 'UOM': 'MSCFH', 'SSVal': 27.5},
    'FF3': {'Name': 'SWS Gas', 'UOM': 'MSCFH', 'SSVal': 8.0}
}

PLOT_LIMITS = {'x': 10, 'y': 600}
...
def render_gain_matrix_inputs():
    ...
    with st.expander('Gain Matrix (MV-CV)'):
        gains = {
            'G11': (-0.1131, 0.02, "%.4f"),
            'G12': (0.00122, 0.001, "%.4f"),
            'G21': (-0.5649, 0.10, "%.4f"),
            'G22': (0.00188, 0.001, "%.4f"),
            'G31': (3.0882, 0.5, "%.4f"),
            'G32': (0.0782, 0.01, "%.4f")
        }
...
def render_feedforward_gains():
    ...
    with st.expander('Feedforward Gains'):
        ff_gains = {
            'G13': (0.08096, 0.1, 1), 'G14': (0.19803, 0.1, 2), 'G15': (0.31035, 0.1, 3),
            'G23': (0.20909, 0.1, 1), 'G24': (0.26042, 0.1, 2), 'G25': (0.14858, 0.1, 3),
            'G33': (9.97798, 1.0, 1), 'G34': (1.42339, 1.0, 2), 'G35': (4.38991, 1.0, 3)
        }
...
def render_ff_disturbances():
    ...
    ff_labels = [
        'AC155.CO % (H2S-2*SO2)',
        'Amine Acid Gas (MSCFH)',
        'SWS Gas (MSCFH)'
    ]
    
    for i, (ff, label) in enumerate(zip(['FF1', 'FF2', 'FF3'], ff_labels), 1):
        var = VARIABLES[ff]
        st.number_input(label, step=0.1, key=f"{ff}SSVal")
...
def render_gain_matrix_tab():
    ...
    with cols[0]:
        ...
        varvals = {
            ...
            f"FF - AC155": [gains['G13'], gains['G23'], gains['G33']],
            f"FF - AAG": [gains['G14'], gains['G24'], gains['G34']],
            f"FF - SWAG": [gains['G15'], gains['G25'], gains['G35']],
        }
    ...
    with cols[0]:
        ...
        st.latex(rf"""
            \begin{{align}}
                \Delta \text{{{VARIABLES['CV1']['Name']}}} &= {gains['G11']:.3f} \cdot \Delta \text{{{VARIABLES['MV1']['Name']}}} + {gains['G12']:.3f} \cdot \Delta \text{{{VARIABLES['MV2']['Name']}}} + \color{{darkblue}}{{{gains['G13']:.3f} \cdot \Delta \text{{AC155.CO}} + {gains['G14']:.3f} \cdot \Delta \text{{AAG}} + {gains['G15']:.3f} \cdot \Delta \text{{SWAG}}}}\\ 
                \Delta \text{{{VARIABLES['CV2']['Name']}}} &= {gains['G21']:.3f} \cdot \Delta \text{{{VARIABLES['MV1']['Name']}}} + {gains['G22']:.3f} \cdot \Delta \text{{{VARIABLES['MV2']['Name']}}} + \color{{darkblue}}{{{gains['G23']:.3f} \cdot \Delta \text{{AC155.CO}} + {gains['G24']:.3f} \cdot \Delta \text{{AAG}} + {gains['G25']:.3f} \cdot \Delta \text{{SWAG}}}}\\
                \Delta \text{{{VARIABLES['CV3']['Name']}}} &= {gains['G31']:.3f} \cdot \Delta \text{{{VARIABLES['MV1']['Name']}}} + {gains['G32']:.3f} \cdot \Delta \text{{{VARIABLES['MV2']['Name']}}} + \color{{darkblue}}{{{gains['G33']:.3f} \cdot \Delta \text{{AC155.CO}} + {gains['G34']:.3f} \cdot \Delta \text{{AAG}} + {gains['G35']:.3f} \cdot \Delta \text{{SWAG}}}}
            \end{{align}}
            """)
